...
void FFTOceanSurface::computeSea( unsigned int totalFrames )
{
	...
	for( unsigned int frame = 0; frame < totalFrames; ++frame )
	{
		...
		if(_mipmapData[frame][0].getMinHeight()<_minHeight)
			_minHeight=_mipmapData[frame][0].getMinHeight();
		if(_mipmapData[frame][0].getMaxHeight()>_maxHeight)
			_maxHeight=_mipmapData[frame][0].getMaxHeight();
...
{
	std::string ocean_surface_vertex,ocean_surface_fragment;
	if(!_loadShaderFromFile)
	{


		ocean_surface_vertex= 
	...
	{
		ocean_surface_vertex  = "water.vert";
		ocean_surface_fragment = "water.frag";
	}

	osg::Program* program = ShaderManager::instance().createProgram("ocean_surface", ocean_surface_vertex, ocean_surface_fragment, _loadShaderFromFile);
	return program;
}
...
void FFTOceanSurface::enableReflections(bool on)
{	
	if(_enableReflections==on)
		return;

	_enableReflections=on;
	osg::ref_ptr<osg::StateSet> ss=getStateSet();
	if(ss.valid())
		ss->getOrCreateUniform("osgOcean_EnableReflections",osg::Uniform::BOOL)->set(_enableReflections);
}
void FFTOceanSurface::enableRefractions(bool on)
{
	...
	osg::ref_ptr<osg::StateSet> ss=getStateSet();
	if(ss.valid())
		ss->getOrCreateUniform("osgOcean_EnableRefractions",osg::Uniform::BOOL)->set(on);
}
// -------------------------------
