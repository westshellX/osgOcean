...
SmuGerstnerWave::SmuGerstnerWave(void) :
	_amplitude(0.1f),
	_amplitudeMul(0.5f),
	_lambda0(14.0f),
	_lambdaMul(1.2f),
...
SmuGerstnerWave::SmuGerstnerWave(float amplitude,
	float amplitudeMul,
	float baseWavelen,
	float wavelenMul,
	float direction,
...
SmuGerstnerWave::SmuGerstnerWave(const SmuGerstnerWave& copy) :
...
void SmuGerstnerWave::updateWaves(float time)
{
...
void SmuGerstnerWave::createWaves(void)
{
	...
	for (int i = 0; i < NUM_WAVES; i++)
	{
		...
		_waves[i].w = 2.f*osg::PI;// sqrt(9.8f*k);
...
void SmuGerstnerWave::packWaves(osg::FloatArray* constants)const
{
	...
	for (unsigned i = 0, j = 0; i < itr; i++, j += 4)
	{
		...
		ptr += 4;
		// amplitude / k
		(*constants)[ptr + 0] = _waves[j + 0].kmod;
		(*constants)[ptr + 1] = _waves[j + 1].kmod;
		(*constants)[ptr + 2] = _waves[j + 2].kmod;
		(*constants)[ptr + 3] = _waves[j + 3].kmod;
		ptr += 4;
		// phase
		(*constants)[ptr + 0] = _waves[j + 0].phi0;
		(*constants)[ptr + 1] = _waves[j + 1].phi0;
		(*constants)[ptr + 2] = _waves[j + 2].phi0;
		(*constants)[ptr + 3] = _waves[j + 3].phi0;
		ptr += 4;
	}
}